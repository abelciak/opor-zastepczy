					...
					for ($iopor=1; $iopor<=13 ; $iopor++)
					{
						...
						//koniec czyszczenia
						$liczsz=1;
						$opornikipokaz=mysql_query("SELECT * from oporniki where galazOpornik='{$idIncludeEdycjaROWGora}' AND polozenie='gora' order by idOpornik ASC");
					...
					for ($iopor=1; $iopor<=13 ; $iopor++)
					{
						...
						//koniec czyszczenia
						$liczsz=1;
						$opornikipokaz=mysql_query("SELECT * from oporniki where galazOpornik='{$idIncludeEdycjaROWDol}' AND polozenie='dol' order by idOpornik ASC");
						while($rowoporniki=mysql_fetch_array($opornikipokaz))
						{
							$opornikAsz[$liczsz]=$rowoporniki["opornikA"];
							$opornikBsz[$liczsz]=sprintf("%02d",  $rowoporniki["opornikB"]);
